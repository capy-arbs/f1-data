...
- **The snapshot record has no timestamp, so its `date` is `NaT`** (`_parse_ts("")`), and it is the *oldest* state in the stream. pandas sorts `NaT` **last**, so any "latest row per driver" reduction (`sort_values("date").groupby(...).tail(1)`) picks the snapshot and freezes that value at connect time for the whole session. Every such reduction must pass `na_position="first"` — `_gap_between`, `latest_intervals` and `latest_positions` do. This only bites the SignalR path; the static archive's session-relative timestamps are never empty, which is why the fixture tests never caught it.
