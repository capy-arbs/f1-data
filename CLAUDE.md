...
- **⚠️ Cloud status (unresolved as of 2026-06-28).** This is **verified working locally** (live P2 *and* the live Austrian GP race) but **unverified on Streamlit Community Cloud** — during the live race the recorder stayed stuck (`is_recording` True, recording file never grew) and a fresh redeploy didn't fix it. Strong suspicion: Community Cloud lets the WSS handshake through but drops the WebSocket data frames (or kills the background thread). The page's `Recorder —` diagnostic line (`f1_signalr.recorder_status`) will settle it on the next live session — `ws connected: True · file: 0 bytes` confirms the muzzle. If confirmed, live-on-Cloud needs an **external recorder** (a small always-on worker streaming the feed to a store the app reads) or local-only; the static-archive + FastF1 paths keep Cloud working for everything that isn't a live session. See `project_notes.md` → Known Issues for the full write-up.
