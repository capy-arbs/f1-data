...
# -- Live content ---------------------------------------------------------
# Everything below is a fragment with its own refresh timer. The previous
# approach slept inside the script run and then called st.rerun(), which left
# the script permanently in its 'running' state — Streamlit fades stale
# elements for the duration of a run, so the page was dimmed ~100% of the
# time, every chart was torn down each tick, and widget clicks queued behind
# the sleep. A fragment reruns only its own block, so the page stays settled
# at full opacity and keeps responding to input between ticks.
@st.fragment(run_every=interval if auto_refresh else None)
def _render_live() -> None:
    # Bypass the fetchers' TTLs on each tick, as the old rerun loop did.
    if auto_refresh:
        for fn in _REFRESH_FNS:
            fn.clear()
    ...
    if auto_refresh:
        st.caption(
            f"Live · refreshing every {interval}s · "
