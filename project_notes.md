...
## Known Issues / To Fix
- **⚠️ OPEN: live SignalR feed on Streamlit Cloud is unverified, suspected blocked.** The websocket recorder works perfectly *locally* (confirmed against the live Austrian GP race, 2026-06-28). On Cloud during the live race it stayed stuck — recorder thread reported connected but the recording file never grew, and a *fresh redeploy* didn't fix it (so it's not process staleness or sleep/wake; the app tab was open since P1). Strong hypothesis: **Community Cloud lets the WSS handshake through but drops/blocks the WebSocket data frames** (or kills the background streaming thread). We never got the decisive reading — the race ended before reloading with the diagnostic. **Next step (zero setup):** during any live session, open the Live page; the new `Recorder —` line (from `f1_signalr.recorder_status`) shows `thread alive / ws connected / file bytes / age / last_error`. `ws connected: True · file: 0 bytes` confirms "Cloud muzzles the websocket". If confirmed, the fix is architectural: move the recorder to a tiny always-on external worker (Fly.io/Railway/cheap VM) that streams the feed and hands it to the app, **or** accept local-only for live. Post-session (static archive) and all historical pages work fine on Cloud regardless.
